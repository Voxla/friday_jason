...
public class SearchOverlayProc5Procedure {
	public static boolean execute(LevelAccessor world, double x, double y, double z) {
		if (!world.getEntitiesOfClass(JasonVoorheesEntity.class, AABB.ofSize(new Vec3(x, y, z), 75, 75, 75), e -> true).isEmpty()) {
			if ((((Entity) world.getEntitiesOfClass(JasonVoorheesEntity.class, AABB.ofSize(new Vec3(x, y, z), 75, 75, 75), e -> true).stream().sorted(new Object() {
				Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
			...
			}.compareDistOf(x, y, z)).findFirst().orElse(null)) instanceof JasonVoorheesEntity _datEntI ? _datEntI.getEntityData().get(JasonVoorheesEntity.DATA_SearchTimer) : 0) < 250
					...
					}.compareDistOf(x, y, z)).findFirst().orElse(null)) instanceof JasonVoorheesEntity _datEntI ? _datEntI.getEntityData().get(JasonVoorheesEntity.DATA_SearchTimer) : 0) >= 140) {
				return true;
			}
		}
		return false;
